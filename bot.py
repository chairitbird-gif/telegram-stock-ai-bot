...
import os
import re
from datetime import time as dtime, timezone as dt_tz
...
HIGH_IMPACT_WORDS = [
    ...
    "rate cut", "rate hike", "fed cuts", "fed hikes", "fed chair", "emergency",
    "cpi", "inflation report", "jobs report", "crash", "plunges", "tumbles",
    "default", "shutdown", "tariff", "sanctions", "recession", "sell-off",
    "selloff", "bank failure", "ceasefire",
]
...
async def check_news_job(context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if MACRO_ALERTS:
        try:
            items = await asyncio.to_thread(fetch_feed, MACRO_QUERY, 10)
            if items:
                first_run = "__macro__" not in data["seen"]
                seen = set(data["seen"].get("__macro__", []))
                new_items = [
                    i for i in items
                    if i["id"] not in seen and is_high_impact(i["title"])
                ]
                data["seen"]["__macro__"] = (
